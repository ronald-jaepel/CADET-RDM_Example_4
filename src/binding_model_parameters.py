...
def create_process(cv_length=30):
    ...
    column = create_column_model(
        component_system, final_salt_concentration, initial_salt_concentration
    )
    ...
    process.cycle_time = (
        t_gradient_start + gradient_duration + duration_post_gradient_wash
    )
    ...
    process.add_event(
        "grad_start", "flow_sheet.inlet.c", c_gradient_poly, t_gradient_start
    )
    process.add_event(
        "grad_end", "flow_sheet.inlet.c", c_post_gradient_wash, t_gradient_end
    )
...
def create_in_silico_experimental_data():
    def save_csv(
        results, directory=None, filename=None, units=None, noise_percentage=5
    ):
        ...
        for unit in units:
            ...
            solution_without_salt = solution_without_salt + (
            ...
            full_solution = np.concatenate(
                [np.atleast_2d(solution_times), solution.T]
            ).T
...
def create_comparator(reference):
    ...
    comparator.add_difference_metric(
        "PeakPosition", reference, "outlet.outlet", components=["A"]
    )
    comparator.add_difference_metric(
        "PeakHeight", reference, "outlet.outlet", components=["A"]
    )
    return comparator
...
if __name__ == "__main__":
    ...
    for cv in [5, 30, 120]:
        ...
        optimization_problem.add_objective(
            ...
            evaluation_objects=[
                process
            ],  # limit this comparator to be applied to only this one process
